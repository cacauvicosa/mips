...
export const MemoryView = BoxView.extend({
    ...
    displayEditor(evt) {
        ...
        div.append($(
            ...
            '<button name="load_memory" type="button" class="btn btn-secondary" title="Load contents">Load</button>' +
            '<input type="file" style="display:none" id="input_memory_load" />' +
            '<button name="save_memory" type="button" class="btn btn-secondary" title="Save contents">Save</button>' +
            '</div>' + 
        ...
        div.find('#input_memory_load').on('change', e => {
            ...
            reader.onload = (e) => {
                ...
                switch(file[0].toString()){
                    case 'hex':
                        length_file++;
                        numbase = 'hex';
                        div.find("select[name=numbase]").val(numbase);
                        break;
                    case 'bin':
                        length_file++;
                        numbase = 'bin';
                ...
                const memdata = this.model.memdata;
                for (let r = 0; (r < words) && (length_file < file.length); r++, length_file++) {
                    if (help.validNumber(file[length_file], numbase)) {
                            const val = help.base2sig(file[length_file], this.model.get('bits'), numbase);
                            memdata.set(r, val);
                            this.model.updateOutputs(r);
                    }
                }
        ...
        div.find("button[name=load_memory]").on('click', e => {
            div.find('#input_memory_load').trigger('click');
        });

        div.find("button[name=save_memory]").on('click', e => {
            const numbase = get_numbase();
            const memdata = this.model.memdata;
            let file_save = numbase + "\n";
            for (let r = 0; r < words; r++) {
                        let val = help.sig2base(memdata.get(r), numbase);
                        file_save += val + "\n"
                        console.log(file_save)
            }

            const blob = new Blob([file_save], {type: "text/plain;charset=utf-8"});
            saveAs(blob, 'memoria.txt');
